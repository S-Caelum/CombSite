...
const Registration: NextPage = (props): JSX.Element => {
  ...
  const [registrationStep, setRegistrationStep] = useState({ firstStep: "block", secondStep: "none" });
  ...
  const registrationValidationHandler : FormEventHandler<HTMLFormElement> = async (e) => {
    e.preventDefault();
    if ((await validate.EmailValidation(e.currentTarget.email.value) && await validate.BirthDayValidation(e.currentTarget.birthDay.value) == true)){
      setRegistrationData({
  ...
  const registrationHandler : FormEventHandler<HTMLFormElement> = async (e) => {
    ...
    if (userValidationCode !== validationCode.Code) {
      ...
      fetch("/api/auth/register", {
        ...
        .then((res) => res.json()).finally(() => router.push("/"))
    }
  }
  ...
  return (
    <>
      <Modal
        ...
        css={{ mb: "20rem", height: "30rem", width: "30rem", borderWidth: "1px", borderColor: "$cyan100" }}>
        ...
        <Card css={{ bgColor: "transparent" }}>
          ...
          <Card.Body css={{ display: `${registrationStep.secondStep}` }}>
            <form onSubmit={(e) => registrationHandler(e)}>
              <Col css={{ display: "flex", flexDirection: "column", justifyContent:"center" }}>
                <Text css={{fontFamily:"Manrope", fs:"$lg"}}> На указанный вами адрес электронной почты было отправлено письмо с кодом подтвеждения. </Text>
                <Spacer y={1.3} />
                <Input
                  ...
                  css={{ "@sm": { ml: "3rem", mr: "3rem" } }} />
                <Spacer y={1.8} />
                <Row css={{ justifyContent: "space-around", flexDirection:"column", "@sm": { flexDirection:"row" } }}>
                  <Button size={"md"} color="secondary" onPress={registrationStepChangeHandler}>
                    Вернуться
                  </Button>
